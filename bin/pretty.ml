...
let fruit =
  ...
  Lwd.join
    (Lwd.map (Lwd.get choice) ~f:(fun current ->
...
let () =
  for _ = 0 to 99 do
    List.iter
      ...
      [
        P.group (string "This" ^/^ string "is" ^/^ string "pretty."); P.hardline
      ; P.ui (Ui.space 0 1); P.hardline
      ...
      ; P.group
          ...
          ^^ spring); P.hardline; P.ui (Ui.space 0 1); P.hardline
      ];
...
let () =
  ...
  Lwd.set base
    (Nottui_widgets.h_pane
       ...
       (Lwd.pure Ui.empty));
  Nottui_miou.run (Nottui_widgets.window_manager_view wm)
